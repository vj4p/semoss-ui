...
/**
 * How a caller hears about a partial failure.
 *
 * This library does not log. Each batch below is tolerated rather than fatal, so
 * something has to say a dimension was lost, and a host is the only thing that
 * knows where that belongs — devtools for a web app, the transcript for a
 * console. Omitting it swallows the failure, which is why every caller passes
 * one.
 */
export type PackReadErrorReporter = (message: string, error: unknown) => void;
...
const batchReadPackTools = async (
	...
	try {
		...
		onError?.("Could not read the packs' tools", e);
	}
	return byPack;
};
...
const batchReadPackDescriptions = async (
	...
	try {
		...
		onError?.("Could not read the packs' descriptions", e);
	}
	return byPack;
};
 ...
 *
 * `MyProjects` filtered to the `MCP` tag is both the discovery query and the
 * access check — a pack the caller cannot read is simply not returned.
 *
 * Each batch is tolerated separately: one pack whose project is mid-deploy costs
 * that dimension (no description, or no counts), not the whole list. A failure
 * of the discovery query itself yields an empty list, since there is nothing to
 * enumerate.
 */
export const readCapabilityPacks = async (
	insight: PixelRunner,
	onError?: PackReadErrorReporter,
	...
	try {
		...
		const [toolsByPack, describedByPack] = await Promise.all([
			batchReadPackTools(insight, ids, onError),
			batchReadPackDescriptions(insight, ids, onError),
		]);
		...
		onError?.("Failed to load capability packs", e);
		return [];
	}
};
